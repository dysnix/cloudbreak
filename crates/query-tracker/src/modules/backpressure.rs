...
//! Backpressure — do not fight the indexer.
//!
...
pub async fn read_indexer_queue_size(metrics_url: &str) -> Option<u64> {
    ...
    let body = client.get(metrics_url).send().await.ok()?.text().await.ok()?;
...
/// `true` when DDL should be deferred: the indexer queue is above `threshold`,
