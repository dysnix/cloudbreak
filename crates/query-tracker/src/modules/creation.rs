...
//! configured [`PriorityMode`] allows, and — if it passes the program
//! allow/deny list — builds the index **pair** (`accounts` +
//! `snapshot_accounts`).
//!
//! There is no in-memory queue: candidates live in the DB and are ranked on
//! read, so nothing is lost across restarts and a full cap simply *slows*
//! creation (the candidate stays) instead of dropping work. Physical `CREATE
//! INDEX IF NOT EXISTS` plus a persisted `created` status means we never re-issue
//! an index we already have — the old `!found` churn is gone.
...
async fn create_one(store: &Store, identity: &IndexIdentity, table: &str) -> Result<(), String> {
    ...
    db.execute(stmt).await.map(|_| ()).map_err(|e| e.to_string())
}
