...
impl PatternRow {
    /// Reconstruct the [`IndexIdentity`] this row represents, or `None` if the
    /// stored program bytes are not a valid pubkey.
    pub fn identity(&self) -> Option<IndexIdentity> {
        let program = Pubkey::try_from(self.program.as_slice()).ok()?;
        Some(IndexIdentity::from_parts(
            program,
