...
//!
//! Body is a [`TrackBatch`]; a single query is just a batch of length one, so
//! the legacy single-track case needs no separate route. On a database error we
...
pub async fn handle(req: Request<Incoming>, state: &Arc<AppState>) -> Response<Full<Bytes>> {
    ...
    match ingest::apply_batch(&state.store, batch).await {
        ...
        Err(e) => {
            ...
            text(StatusCode::INTERNAL_SERVER_ERROR, "failed to persist observations")
        }
    }
}
