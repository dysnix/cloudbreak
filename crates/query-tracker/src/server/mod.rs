...
//!
//! Two servers run on the existing two ports:
//! - main (`server_addr`): `POST /track` ingest + `GET /debug/*` introspection,
//! - operational (`metrics_addr`): `GET /metrics` + `GET /health`.
...
/// Serve the main API (ingest + debug) until the process exits.
...
async fn route(req: Request<Incoming>, state: Arc<AppState>) -> Response<Full<Bytes>> {
    match (req.method(), req.uri().path()) {
        (&Method::POST, cloudbreak_core::modules::query_tracker_api::TRACK_PATH) => {
            track::handle(req, &state).await
        }
        (&Method::GET, "/debug/candidates") => debug::candidates(&state).await,
        (&Method::GET, "/debug/created") => debug::created(&state).await,
        (&Method::GET, "/debug/discrepancies") => debug::discrepancies(&state).await,
