...
6. If `max-auto-indexes` is set and the `snapshot_accounts` table has reached that cap, index creation is **paused** until eviction makes room again. Candidates are kept queued in `index_patterns` with no loss of data.
...
- `max-auto-indexes` is set **and** the `snapshot_accounts` table is at/above `eviction-fill-threshold` — below the fill line nothing is dropped;
...
- the drop is a **net gain** — the slot it frees would be refilled by a strictly more valuable pending creation (the *value guard*, below).

When over the fill line(`eviction-fill-threshold`), eligible pairs are dropped in ascending `priority-mode` order (least worth keeping first) just until the table is back at the target.

**Value guard.** An eviction only pays off if the freed slot is refilled with something worth more, so each drop is weighed against the creation it would make room for — both scored by the same `priority-mode`. The least-useful eviction candidate is paired with the top creation candidate, the 2nd-least with the 2nd, and so on; a pair is dropped only when `evict_score < create_score`. Because eviction candidates ascend in score and creation candidates descend, the first pair that fails means none after it can pass, so the pass stops there — likewise if no pending creation is left to justify freeing another slot. As a result the table may sit above the fill target when there is nothing worth swapping in, which is intended: evicting for its own sake only sheds value. The guard inherits the mode's notion of "value" — lifetime-total modes let a once-hot idle index resist eviction, while `weighted` weighs recent windowed activity so decisions track current throughput.

**Latency regression guard.** The tracker records each pattern's average query latency **with** the index versus **without** it (cost is routed into a with- or without-index bucket by the pattern's status at record time). When `index-regression-guard` is not `off`, the eviction pass — before the supply refresh, and **regardless of the fill threshold**, since a harmful index should go even when there is room — flags any created index that is slower with the index than without by more than `index-regression-ratio`, once the index is older than `index-min-age-grace` (long enough to have gathered with-index latency to compare against its frozen without-index baseline). In `warn` mode it logs and reports the current count via the `query_tracker_regressed_indexes` gauge; in `evict` mode it also drops the pair and marks the pattern `rejected` (so that gauge stays near zero and the count shows up under `query_tracker_patterns{status="rejected"}` instead). A `rejected` pattern is **not** rebuilt by demand alone — only after it has been rejected for at least `index-regression-retry-delay` (time spent gathering fresh without-index samples) *and* its without-index average since rejection has climbed past the with-index average captured at rejection (times `index-regression-ratio`) does the creation loop return it to `candidate`. Both the detection and the recovery comparison first scale the without-index average by `without-index-compensation-factor`, so a without-index scan whose wall-clock is deflated by parallel workers (fast in elapsed time, but far heavier in CPU/IO) is not mistaken for a cheaper alternative and does not push a genuinely useful index into a false regression. The same latency signal optionally feeds the `weighted` score via `latency-weight`, and the per-index averages are visible on `/debug/created`.

Discrepancy detection (`discrepancy-enabled`) and `EXPLAIN` sampling (`explain-enabled`) are **observational only**: they flag/log when API demand and Postgres `idx_scan` diverge, or when the planner would not use an index, surfacing problems on `/debug/*` and `/metrics`. The `EXPLAIN` probe is built from the row's stored `example_request` — a real request captured on first sight — so its memcmp constants are realistic values the planner has statistics for (it falls back to zeros per column when no example is stored yet), and it runs on **both** physical tables (`accounts` and `snapshot_accounts`), one log line each. Because those tables are partitioned, the probe matches the plan against the index's partition-tree members (the auto-named per-partition child indexes that `EXPLAIN` actually prints), not the partitioned parent's name — which is never scanned directly and would otherwise make every index look unused. Each pass folds the two per-table verdicts into one `explain_state` (`none` / `accounts_table` / `snapshot_accounts_table` / `both`), persists it to the pattern (surfaced on `/debug/created`), and refreshes the `query_tracker_explain_state{state}` gauge with the per-state counts of the latest run; the "would not use" warning carries the pattern's `compensated_idx_scan` so a contradiction (planner says unused, yet clearly scanned) is obvious inline. That example request is also surfaced as `example_request` on the `/debug/created` and `/debug/candidates` views (with `?example=true`) for documentation/sampling. They never create or drop anything and do not gate eviction. Note a served `getProgramAccounts` is a `UNION ALL` over `accounts` + `snapshot_accounts`, so it scans **both** indexes of a pair — raw `idx_scan` runs ~2× the request count. The tracker divides supply by that factor (`SCANS_PER_REQUEST = 2`) before comparing it to demand, so a healthy index sits near a 1:1 ratio.
...
The three `/debug/*` endpoints share query-string conventions. Latencies are rendered in **ms (2 dp)** (`*_ms`), index size in **MiB (2 dp)** (`index_mb`), and supply as `compensated_idx_scan` (raw `idx_scan ÷ 2`, since a served GPA scans both tables of the pair). Every response is `{ "total", "count", "limit", "<items>": [...] }`. Common parameters:
...
- Order keys: `/candidates` → `score` (default), `demand_count`, `avg_cost_ms`, `variety_estimate`; `/created` → `created_at` (default, newest first), `index_mb`, `demand_count`, `idx_scan`, `avg_cost_with_index_ms`, `latency_gain`, `variety_estimate`; `/discrepancies` → `discrepancy_ratio` (default, `dir=asc` = most-starved first) with `min_ratio` / `max_ratio` bounds.
- `/created` also accepts `filter=eviction_candidates` to show only the indexes the eviction pass would consider (past the idle + age-grace gates), each annotated with its `eviction_score` and ordered least-useful-first by default (`order=score`). The real drop still depends on the fill threshold and value guard at runtime.
