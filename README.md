...
The three `/debug/*` endpoints share query-string conventions. Latencies are rendered in **ms (2 dp)** (`*_ms`), index size in **MiB (2 dp)** (`index_mb`), and supply as `compensated_idx_scan` (raw `idx_scan ÷ 2`, since a served GPA scans both tables of the pair). Each row's `score` (the shared `priority-mode` ranking) is shown in **thousands, 1 dp** (e.g. `"37.2K"`) — the raw score derives from microsecond costs, so ÷ 1000 makes it proportional to the `*_ms` figures; every response repeats this in a top-level `docs` field. Every response is `{ "total", "count", "limit", "docs", "<items>": [...] }`. Common parameters:
