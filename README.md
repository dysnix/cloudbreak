...
- `/created` also accepts `filter=eviction_candidates` to show only the indexes the eviction pass would consider (past the idle + age-grace gates), each annotated with its `eviction_score` and ordered least-useful-first by default (`order=score`). The real drop still depends on the table being above the fill target at runtime — eviction only trims the buffer band back to the target.
